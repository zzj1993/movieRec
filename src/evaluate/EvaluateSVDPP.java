...
public class EvaluateSVDPP {
	...
	public EvaluateSVDPP() throws ClassNotFoundException, SQLException {
		/**************svd**************/
	...
	public void MAE_RMSE_svd() throws ClassNotFoundException, SQLException{
		conn = DBUtil.getConn();
		pst = conn.prepareStatement("SELECT movieid from svd where score>=4");
		rs = pst.executeQuery();
		while(rs.next()){
		...
		pst = conn.prepareStatement("SELECT sum(abs(a.score-b.score)),sum(pow(a.score-b.score,2)),count(*) from svd a,test b where a.movieid=b.movieid and a.userid=b.userid");
		rs = pst.executeQuery();
		while(rs.next()){
			sum1 = rs.getDouble(1);
			sum2 = rs.getDouble(2);
