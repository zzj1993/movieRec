...
public class EvaluateUserBased {
	...
	public EvaluateUserBased() throws ClassNotFoundException, SQLException {
		/**************user-base**************/
		System.out.println("*********user-base*********");
		MAE_RMSE_user();
		recall_precision_user();
	}
	
	public void MAE_RMSE_user() throws ClassNotFoundException, SQLException{	
		conn = DBUtil.getConn();
		pst = conn
				.prepareStatement("SELECT sum(abs(a.score-b.score)),sum(pow(a.score-b.score,2)),count(*) from u2 a,test b where a.movieid=b.movieid and a.userid=b.userid");
		rs = pst.executeQuery();
		while (rs.next()) {
			sum1 = rs.getDouble(1);
			sum2 = rs.getDouble(2);
		...
		// recall-precision
		pst = conn.prepareStatement("SELECT movieid from u2 where score>=4");
		rs = pst.executeQuery();
		while (rs.next()) {
	...
	public void recall_precision_user(){
		result.clear();
		result.addAll(list1);
