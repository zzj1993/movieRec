...
public class Compare {
	...
	public static void MAE_RMSE_recall_precision() throws ClassNotFoundException, SQLException{
		...
		rs1 = pst1.executeQuery();
		while(rs1.next()){
			list2.add(rs1.getInt(1));
		}
		pst1 = conn.prepareStatement("SELECT movieid from slope_weight where score>=4");
		rs1 = pst1.executeQuery();
		while(rs1.next()){
			list3.add(rs1.getInt(1));
		}
		pst1 = conn.prepareStatement("SELECT movieid from slope_weight where score<4");
		rs1 = pst1.executeQuery();
		while(rs1.next()){
			list4.add(rs1.getInt(1));
		}
		
		pst1 = conn.prepareStatement("SELECT sum(abs(a.score-b.score)),sum(pow(a.score-b.score,2)),count(*) from slope_weight a,test b where a.movieid=b.movieid and a.userid=b.userid");
		rs1 = pst1.executeQuery();
		while(rs1.next()){
	...
	//weightless
	public static void MAE_RMSE_recall_precision1() throws ClassNotFoundException, SQLException{
		long t1 = System.currentTimeMillis();
		...
		pst1 = conn.prepareStatement("SELECT movieid from slope_weightless where score>=4");
		rs1 = pst1.executeQuery();
		while(rs1.next()){
			list3.add(rs1.getInt(1));
		}
		...
		pst1 = conn.prepareStatement("SELECT sum(abs(a.score-b.score)),sum(pow(a.score-b.score,2)),count(*) from slope_weightless a,test b where a.movieid=b.movieid and a.userid=b.userid");
		rs1 = pst1.executeQuery();
		while(rs1.next()){
			sum1 = rs1.getDouble(1);
			sum2 = rs1.getDouble(2);
